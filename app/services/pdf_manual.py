...
def register_font() -> str:
    ...
    # 所有候选字体都失败，使用 Helvetica（不支持 CJK）
    # 注意：Helvetica 是 ReportLab 内置字体，不需要注册
    return "Helvetica"
...
def generate_manual_pdf(product_id: str, languages: Optional[list[str]] = None) -> Path:
    ...
    如果字体注册失败，会降级到 Helvetica（不支持 CJK 字符）。
    """
    import logging
    ...
    if font_name == "Helvetica":
        logging.warning(
            "No suitable font found. Using Helvetica (CJK characters may not render correctly). "
            "Install fonts: macOS: Arial Unicode, Windows: MS YaHei, Linux: Noto Sans CJK"
        )
    ...
    try:
        ...
        logging.error(f"Failed to generate PDF for {product_id}: {e}")
        # 清理可能生成的不完整文件
        if path.exists():
            path.unlink()
