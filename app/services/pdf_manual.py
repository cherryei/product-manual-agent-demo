...
def generate_manual_pdf(product_id: str) -> Path:
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    product = get_product(product_id)
    font_name = register_font()
    styles = _styles(font_name)
    path = OUTPUT_DIR / f"{product_id}-manual-7-languages.pdf"
    ...
    story: list = [
        Spacer(1, 2.4 * cm),
        Paragraph(en["name"], styles["cover_title"]),
        Paragraph("Multilingual Product Manual / EN DE IT FR ES JP CN", styles["cover_subtitle"]),
        Spacer(1, 0.5 * cm),
        SvgPlaceholder("Solid wood table product image", 15.5 * cm, 8.0 * cm),
        Spacer(1, 0.8 * cm),
        Paragraph(en["summary"], styles["cover_subtitle"]),
        Spacer(1, 0.3 * cm),
        Paragraph(
    ...
    for index, language in enumerate(SUPPORTED_LANGUAGES):
        ...
        story.append(PageBreak() if index < len(SUPPORTED_LANGUAGES) - 1 else Spacer(1, 0.1 * cm))
